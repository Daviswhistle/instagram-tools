    ...
    def acquire(cls, path: Path) -> InstanceLock:
        ...
        try:
            ...
            raise AppAlreadyRunningError(
                "Instagram 도구가 이미 실행 중입니다. 기존 앱과 전용 Chrome 창을 닫은 뒤 다시 실행하세요."
            ) from exc
...
class Storage:
    ...
    @staticmethod
    def _root_has_user_state(root: Path) -> bool:
        if (root / "config.json").is_file() or (root / "non_follower_cleaner.json").is_file():
            return True

        profile = root / "chrome-profile"
        try:
            next(profile.iterdir())
        except (StopIteration, FileNotFoundError, NotADirectoryError, OSError):
            return False
        return True
    ...
    def default(cls) -> Storage:
        ...
        if sys.platform == "win32":
            ...
            preferred_root = base / "InstagramTools"
            legacy_root = base / "FollowingAutoLiker"
            ...
            preferred_root = base / "instagram-tools"
            legacy_root = base / "following-auto-liker"

        # Never let an empty/partial new directory hide a working legacy login.
        # Once the product-level location contains real user state it becomes
        # authoritative; otherwise an existing legacy installation is preserved.
        if cls._root_has_user_state(preferred_root):
            root = preferred_root
