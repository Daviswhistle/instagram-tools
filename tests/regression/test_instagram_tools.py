...
from instagram_tools.app import (
    InstagramToolsApp,
    use_compact_layout,
    window_height_for_screen,
)
from instagram_tools.auto_like_browser import INSTAGRAM_HOME_URL
from instagram_tools.auto_like_engine import InstagramRestrictionError, ScanSummary
from instagram_tools.friendship_browser import SharedChromeBrowserSession, VerifiedFriendshipBackend
from instagram_tools.non_followers import (
    FriendshipRequestError,
    NonFollowerCleanerError,
    OperationStopped,
)
from instagram_tools.storage import AppConfig
from instagram_tools.worker import InstagramAutomationWorker
...
class PackagedEntryRegressionTestCase(unittest.TestCase):
    def test_entry_script_imports_without_package_context(self) -> None:
        repository_root = Path(__file__).resolve().parents[2]
        namespace = runpy.run_path(
            str(repository_root / "instagram_tools/__main__.py"),
...
class PersistentWorkerRegressionTestCase(unittest.TestCase):
    ...
    def test_stopped_auto_like_publishes_partial_summary_and_timestamp(self) -> None:
        ...
        with patch("instagram_tools.worker.FollowingFeedScanner", StoppedScanner):
    ...
    def test_restriction_publishes_partial_auto_like_summary_and_timestamp(self) -> None:
        ...
        with patch("instagram_tools.worker.FollowingFeedScanner", RestrictingScanner):
